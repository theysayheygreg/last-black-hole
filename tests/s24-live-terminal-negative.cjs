...
assert.match(fs.readFileSync(path.join(ROOT, "scripts", "sim-runtime.cjs"), "utf8"), /maxConnections:\s*16\s*,/);
const profiles = JSON.parse(fs.readFileSync(path.join(ROOT, "src", "content", "session-profiles.data.json"), "utf8"));
const deepFieldProfileId = profiles.MAP_SESSION_PROFILES["deep-field"];
assert.strictEqual(profiles.SESSION_PROFILES[deepFieldProfileId].maxScavengers, 7);
