  ...
  window.__TEST_API = {
    ...
    showUiFixture(surface, options = {}) {
      ...
      if (name === 'title') {
        ...
        state.setTitleLayoutForTest?.(options.layout ?? 'center');
        state.setTitleTimerForTest?.(options.titleTimer ?? 1.2);
        state.setTitleLoopTimeForTest?.(options.loopTime ?? 0);
        return true;
      }
    ...
    setInhibitorVisualStateForTest(state) {
      const { setInhibitorVisualStateForTest } = getState();
      return typeof setInhibitorVisualStateForTest === 'function'
        ? setInhibitorVisualStateForTest(state)
        : false;
    },
