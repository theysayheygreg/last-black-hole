...
export function createPresentationSceneSource(input = {}) {
  ...
  return {
    ...
    waveRings: (world.waveRings || []).filter((ring) => ring.alive !== false).map((ring, index) => ({
      ...
      eventId: ring.eventId || ring.id || `wave-${index}`,
      cause: ring.cause || 'unknown',
      ...
      telegraphStartTime: ring.telegraphStartTime,
      state: ring.state || 'active',
    })),
    ...
    noiseEmitters: (world.noiseEmitters || []).filter((emitter) => Number(emitter.radiusMeters) > 0).map((emitter, index) => ({
      id: emitter.id || `world-noise-${index}`,
      wx: emitter.wx,
      wy: emitter.wy,
