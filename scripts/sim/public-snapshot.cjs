...
function projectPlayer(player, facts) {
  ...
  const slingshot = player.slingshot ? {
    ...
    entrySpeed: player.slingshot.entrySpeed || 0,
    arcSpeed: player.slingshot.arcSpeed || 0,
    boost: player.slingshot.boost || 0,
...
function projectWorld({
  ...
  return {
    ...
    waveRings: waveRings.map((ring) => ({
      ...
      radius: ring.radius,
      previousRadius: ring.previousRadius ?? ring.radius,
      frontWidth: ring.frontWidth ?? null,
      amplitude: ring.amplitude,
      initialAmplitude: ring.initialAmplitude,
      sourceWellId: ring.sourceWellId ?? null,
      launchTime: ring.launchTime ?? null,
      telegraphStartTime: ring.telegraphStartTime ?? null,
      state: ring.state || (ring.alive === false ? "expired" : "active"),
