...
function buildRunEntry({ profile, player, outcome, runDuration, session, runResult, result }) {
  ...
  const entry = {
    ...
    ecologyPhaseReached: runResult?.ecologyPhaseReached ?? null,
    ecologyKindsReached: Array.isArray(runResult?.ecologyKindsReached) ? [...runResult.ecologyKindsReached] : [],
    ecologyCounts: runResult?.ecologyCounts ? { ...runResult.ecologyCounts } : {},
    emEarned,
    emCredited: emEarned,
    tax: result.tax || 0,
