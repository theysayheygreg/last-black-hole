...
const LOG_FILE = path.join(TMP, "dev-server.log");
...
async function start() {
  ...
  if (isAlive(existingPid)) {
    const meta = readMeta();
    console.log(`LBH dev server already running on ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${existingPid}).`);
    return;
  }
  ...
  if (!up) {
    console.error(`LBH dev server did not start cleanly on port ${PORT}. See ${LOG_FILE}.`);
    process.exit(1);
  }

  const meta = readMeta();
  console.log(`LBH dev server running at ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${meta?.pid || child.pid}).`);
...
async function stop() {
  ...
  if (!pid || !isAlive(pid)) {
    cleanupFiles();
    console.log("LBH dev server is not running.");
    return;
  }
  ...
  while (Date.now() < deadline) {
    if (!isAlive(pid)) {
      cleanupFiles();
      console.log(`Stopped LBH dev server (pid ${pid}).`);
      return;
    }
    await sleep(100);
  }
  ...
  cleanupFiles();
  console.log(`Force-stopped LBH dev server (pid ${pid}).`);
}
...
function status() {
  ...
  if (pid && isAlive(pid)) {
    console.log(`LBH dev server is running at ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${pid}).`);
    console.log(`Log: ${LOG_FILE}`);
    return;
  }

  if (listener) {
    console.log(`LBH dev server is not running, but port ${PORT} is occupied by ${listener.command} (pid ${listener.pid}).`);
    return;
  }
