...
const PID_FILE = path.join(TMP, `control-plane-${PORT}.pid`);
const META_FILE = path.join(TMP, `control-plane-${PORT}.json`);
const LOG_FILE = path.join(TMP, `control-plane-${PORT}.log`);
const SERVER_SCRIPT = path.join(ROOT, "scripts", "control-plane-runtime.js");
...
async function start() {
  ...
  if (isAlive(existingPid)) {
    const meta = readMeta();
    console.log(`LBH control plane already running at ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${existingPid}).`);
    return;
  }
  ...
  if (!up) {
    console.error(`LBH control plane did not start cleanly on port ${PORT}. See ${LOG_FILE}.`);
    process.exit(1);
  }

  const meta = readMeta();
  console.log(`LBH control plane running at ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${meta?.pid || child.pid}).`);
...
async function stop() {
  ...
  if (!pid || !isAlive(pid)) {
    cleanupFiles();
    console.log("LBH control plane is not running.");
    return;
  }
  ...
  while (Date.now() < deadline) {
    if (!isAlive(pid)) {
      cleanupFiles();
      console.log(`Stopped LBH control plane (pid ${pid}).`);
      return;
    }
    await sleep(100);
  }
  ...
  cleanupFiles();
  console.log(`Force-stopped LBH control plane (pid ${pid}).`);
}
...
function status() {
  ...
  if (pid && isAlive(pid)) {
    console.log(`LBH control plane is running at ${meta?.url || `http://${HOST}:${PORT}/`} (pid ${pid}).`);
    console.log(`Log: ${LOG_FILE}`);
    return;
  }

  if (listener) {
    console.log(`LBH control plane is not running, but port ${PORT} is occupied by ${listener.command} (pid ${listener.pid}).`);
    return;
  }
