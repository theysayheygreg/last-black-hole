...
| v0.4 multiplayer architecture plan | `docs/v0.4/ROADMAP.md` | Active on `codex/v0.4-multiplayer-architecture`; S20 negotiated compression remains the 1–4 product path and eight remains rejected. S23 proves one immutable public body and real cohort reuse can recover eight to 9.00/9.10 Hz in NORMAL, but 88.58/88.33 ms p95 and 95.05/94.63 ms p99 fail the absolute tail gates; four also misses its p95 gate in one round and one regresses S20 cost. Keep S23 default-off, do not merge backward, and do not infer hosted/heavy-sim/24–96 capacity. |
