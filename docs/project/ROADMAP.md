...
| v0.4 multiplayer architecture plan | `docs/v0.4/ROADMAP.md` | Active on `codex/v0.4-multiplayer-architecture`; S20 negotiated compression admits 1–4, while eight remains rejected. S22 implemented, rejected, and reverted the public-only runtime worker seam: two/four workers regress the real clock despite exact parity and authority fencing, and red-team found maintenance liabilities in the disabled integration. The next bounded lane is a shared immutable public body plus recipient-local lineage envelope. Do not merge backward or infer hosted/heavy-sim/24–96 capacity. |
