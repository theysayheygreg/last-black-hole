...
## Current v0.4 Status (2026-07-14)

**Active next-version goal:** turn the admitted v0.4 S20 one-through-four path
into a coherent human multiplayer game. Crew Muster plus the private
room/readiness journey are live and proven with four admitted browsers and a
...
recovery. Terminal recovery failure is now persistent, stops further gameplay
dispatch, neutralizes stale movement, and offers an immediate local return to
launch while the authority retains the body through expiry. A canonical public
crew result is now live beside owner-private cargo/reward detail, and terminal
pilots stay attached until it arrives. Exact settlement, result residency, and
party-preserving rematch lineage are now live. The immediate slice is P5E:
player-facing rematch/wait/leave controls and human evidence without debug
navigation. See
...
- [ ] Four-human product journey and named host/join/extract/death/rematch pack
...
| v0.4 four-human multiplayer product | `docs/v0.4/FOUR-HUMAN-PRODUCT-PLAN.md` | Active on `codex/v0.4-multiplayer-product`; architecture and costing are closed, S20 is the admitted 1–4 path, eight remains rejected, private-room/front-door/reconnect is green, P4 continuity is proven, and P5A-P5D now cover canonical crew results, exact settlement, result residency, and party-preserving rematch lineage. P5E player controls and Greg's four-human gate remain. Do not infer public hosted or 24–96 capacity. |
...
1. **v0.4 Four-Human Multiplayer Product** — Crew Muster, invitations and
   readiness, shared-run readability, reconnect clarity, result/rematch, and a
   four-human Greg gate on the admitted S20 path.
2. **Attract Mode + UI/VFX Identity** — title layout, corruption behavior, UI
   ...
   fixtures, `src/ui/motion.js`, first-pass title/menu/result motion, Home
   instrument-console composition, and Map Select drop-briefing composition.
   Next: HUD/pause/profile polish plus richer route/objective language before
   another motion-only pass.
3. **Feel + Route Pass** — slingshot/speed tuning, map route redesign, spawn
   safety, and remote/local movement parity checks.
4. **Entity Visual Language** — replace bridge primitives for ships, stars,
...
5. **Loop + Meta Clarity** — results, vault/rig/loadout, profile/home,
   chronicle, upgrade write-back, EM balance, and "what changed after this
   run?" readability.
6. **Playable Build Targets** — local source build, Steam Deck desktop/gaming
   mode, itch package shape, public play instructions, release artifacts, and
   current build-status truth.
7. **Process + Harness** — fresh browser/sim rules, visual fixtures, UI
...
- Readability precedes timing. If a screen fails its static selected-action,
  ...
  current ordered targets are HUD, pause, profile destructive states, and route
  objective clarity.
