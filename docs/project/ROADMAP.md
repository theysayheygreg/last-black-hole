...
| v0.4 multiplayer architecture plan | `docs/v0.4/ROADMAP.md` | Active on `codex/v0.4-multiplayer-architecture`; S20 negotiated compression admits 1–4, while eight remains DILATED and rejected. S21 attributes the eight-player clock to serial per-recipient public projection and selects only a feature-flagged runtime public-only projection-worker pilot with authority-owned owner data, selection, compression, ledgers, ordering, and send commit. Do not merge backward or infer hosted/heavy-sim/24–96 capacity. |
