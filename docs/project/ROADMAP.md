...
| v0.4 multiplayer architecture plan | `docs/v0.4/ROADMAP.md` | Active on `codex/v0.4-multiplayer-architecture`; canonical S11 converged positional evidence is method-valid and product-rejected. One client passes, four fails cadence/normalized mean/overload, and eight fails cadence/burst/normalized traffic/clock/correctness. Continue with additional positional schema cleanup; do not merge backward or begin hosted/AOI work from this result. |
