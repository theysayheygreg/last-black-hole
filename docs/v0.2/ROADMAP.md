...
| External engine research | Carbon Engine source read is now captured in `docs/reference/CARBON-ENGINE-RESEARCH.md` | Apply the findings through LBH-sized tickets: sim event journal, snapshot/rebase tests, render-plan descriptor, material registry, route graph fixture, manifests, and audio priority budgets | Revisit only as source-level reference for native/console renderer probes, not as a v0.2 engine migration |
