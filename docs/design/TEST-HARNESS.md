...
| `npm run test:multiplayer-structure` | v0.4 membership/connection fencing, owner-private projection, public history, bounded admission/resume tickets, authenticated idempotent settlement, executor parity, and field-revision lineage. |
| `npm run test:multiplayer-authority` | v0.4 structure plus deterministic 1/4/8-human shared truth, privacy, bytes, tick, heap, result replay, real same-process WebSocket authority, bounded idempotent actions/events, membership-private recovery, reset lineage, executor parity, and field revision. |
...
| `multiplayer-structure` | membership, connection fencing, privacy projection, public history, bounded tickets, settlement idempotency/authentication, executor parity, and field lineage | WAN feel, hosted capacity, or natural multiplayer journeys |
| `multiplayer-authority` | structure plus deterministic 1/4/8-human truth, real same-process WebSocket authority, idempotent actions/events, membership privacy, reset-lineage accounting, baseline measurements, executor parity, and field lineage | WAN loss/reorder soak or a claim of public playability |
| `multiplayer-network` | strict JSON frames, exact byte limits, reliable delivery ids, state coalescing, queue bounds, action receipt/retry/conflict behavior, run-qualified journal replay/rebase, issued-only playback ACKs, real-socket backpressure, cadence, reset lineage, executor parity, field lineage, dual-transport SimClient reconnect/cancellation semantics, and the pure seeded frame-scheduler kernel | WAN feel, natural multi-browser play, TCP/IP loss, or hosted capacity |
| `multiplayer-playable` | real private-room browser journey with unavailable-authority offline recovery, four admitted humans, rejected fifth, copy/paste invitation, readiness, shared launch, owner privacy, reconnect continuity, 1280x800 evidence, and process cleanup | WAN, TLS edge, hosted capacity, shared-run taste/readability approval, or public readiness |
