...
55.905 ms projection/publish p95 against a 55 ms maximum. No sealed 20-second
capture was run. S20 remains the admitted one-through-four product path, eight
remains closed for v0.4, and the low-count replication staircase is complete.

Exactly one next lane is selected: internet-shaped four-player S20
productization. A future eight-player effort requires a new version decision
and materially different architecture hypothesis.
