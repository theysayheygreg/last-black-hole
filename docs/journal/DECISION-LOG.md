...
### Q: What limits the eight-player authority clock, and may workers help?

| Date | Decision |
|------|----------|
| Jul 13 | Keep the sealed S20 rounds as product truth. Profiler-on A1/final-A2 reproduce 3.75/3.70 Hz, 0.747/0.747 authority core, and about 188/199 ms projection p95, while same-source profiler-off B reaches 4.95 Hz and 123 ms. The profiler is diagnostic and materially expensive. |
| Jul 13 | Attribute the critical path to serial per-recipient public core/projection/delta construction. In final A2 at eight it accounts for 77.15 visible ms/beat; sim tick p95 is 1.47 ms, and compression + queue + send call + ACK are individually too small to recover the clock. Inclusive/nested rows and overlapping async callbacks are not summed. |
| Jul 13 | Permit a worker to compute canonical public keyframe/delta bytes only. Owner data, pair choice, compression, ACK/base and retained-wire ledgers, epoch rotation, ordering, queue ownership, socket send, and final commit stay in the one dedicated logical authority per match. Concurrent matches still multiply that authority horizontally. |
| Jul 13 | Select a feature-flagged runtime public-only projection-worker pilot with live issued-request fencing and inline fallback. The synthetic 234-job feasibility corpus has zero mismatch and reduces eight-recipient batch p95 from 66.24–68.57 ms inline to 37.30–37.45 ms with two workers and 27.73–29.33 ms with four across a topology Latin square. This is directional, not admission. |
| Jul 13 | Keep eight rejected. The pilot must sustain every recipient at >=9 Hz in NORMAL with <=100 ms projection p95, exact S20 semantics/traffic, and bounded total CPU/memory/clone cost in counterbalanced profiler-off evidence. |
