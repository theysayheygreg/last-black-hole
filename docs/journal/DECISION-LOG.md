...
| Date | Decision |
|------|----------|
| Jul 13 | Keep the sealed S20 rounds as product truth. Profiler-on A1/A2 reproduce 3.75 Hz and about 188/187 ms projection p95, while same-source profiler-off B reaches 4.95 Hz and 123 ms. The profiler is diagnostic and materially expensive. |
| Jul 13 | Attribute the critical path to serial per-recipient public core/projection/delta construction. At eight it accounts for 75.06 visible ms/beat; sim tick p95 is 1.41 ms, and compression + queue + send call + ACK are individually too small to recover the clock. Inclusive/nested rows and overlapping async callbacks are not summed. |
...
| Jul 13 | Select a feature-flagged runtime public-only projection-worker pilot with live issued-request fencing and inline fallback. The synthetic 234-job feasibility corpus has zero mismatch and reduces eight-recipient batch p95 from 55.69–57.23 ms inline to 35.75–37.74 ms with two workers and 26.96–29.13 ms with four across a topology Latin square. This is directional, not admission. |
